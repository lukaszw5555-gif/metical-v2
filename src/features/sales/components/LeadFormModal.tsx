...
export default function LeadFormModal({ onSubmit, onClose, profiles }: LeadFormModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-t-2xl sm:rounded-2xl max-h-[90dvh] flex flex-col shadow-2xl animate-in slide-in-from-bottom sm:slide-in-from-bottom-0 sm:zoom-in-95">
        <div className="sticky top-0 bg-white border-b border-surface-200 px-5 py-4 flex items-center justify-between rounded-t-2xl sm:rounded-t-2xl shrink-0 z-10">
          <h2 className="text-base font-semibold text-gray-900">Nowy lead</h2>
          <button type="button" onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-lg text-muted-400 hover:bg-surface-100 hover:text-muted-600 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto flex flex-col">
          ...
          <div className="sticky bottom-0 bg-white p-5 border-t border-surface-100 pb-[calc(1.25rem+env(safe-area-inset-bottom))] shrink-0 z-10 sm:rounded-b-2xl">

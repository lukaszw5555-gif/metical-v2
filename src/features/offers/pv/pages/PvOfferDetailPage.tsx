...
import {
  PV_OFFER_STATUS_LABELS, PV_OFFER_STATUS_COLORS,
  PV_STRUCTURE_TYPES, PV_ROOF_TYPES, PV_INSTALLATION_TYPES,
} from '../types/pvOfferTypes';
import {
  Loader2, AlertCircle, Pencil, Sun, User, Phone, Mail, MapPin,
  Zap, Hash, ExternalLink, FileText, Construction,
} from 'lucide-react';
...
export default function PvOfferDetailPage() {
  ...
  const lookupLabel = (list: { value: string; label: string }[], val: string | null) =>
    list.find(i => i.value === val)?.label || val || '—';

  if (loading) {
    return (
      <>
        <PageHeader title="Oferta PV" showBack />
      ...
      <div className="px-4 py-4 mx-auto max-w-lg md:max-w-3xl pb-24 md:pb-8 space-y-4">
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => navigate(`/sales/offers/pv/${offer.id}/edit`)}
          ...
          {offer.valid_until && (
            <p className="text-xs text-muted-500 mt-2">Ważna do: {fmtDate(offer.valid_until)}</p>
          )}
        ...
        <div className="card p-4">
          ...
          <div className="space-y-2">
            <Row icon={<Zap size={14} className="text-muted-400" />} label="Cena netto" value={fmtCurrency(offer.price_net)} />
            ...
            <div className="flex items-center gap-3 p-2 rounded-lg bg-primary-50">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[11px] text-primary-500 leading-tight">Cena brutto</p>
                <p className="text-base font-bold text-primary-700">{fmtCurrency(offer.price_gross)}</p>
              </div>
            </div>
            {offer.margin_value != null && <Row icon={<Hash size={14} className="text-muted-400" />} label="Marża (PLN)" value={fmtCurrency(offer.margin_value)} />}
            {offer.margin_percent != null && <Row icon={<Hash size={14} className="text-muted-400" />} label="Marża (%)" value={`${offer.margin_percent}%`} />}
          </div>
        </div>
        ...
        {(offer.offer_note || offer.internal_note) && (
          <div className="card p-4">
            ...
            {offer.offer_note && (
              <div className="mb-3">
                <p className="text-[11px] text-muted-400 mb-1">Notatka ofertowa</p>
                <p className="text-sm text-gray-700 whitespace-pre-wrap">{offer.offer_note}</p>
              </div>
            )}
            {offer.internal_note && (
              <div>
                <p className="text-[11px] text-muted-400 mb-1">Notatka wewnętrzna</p>
                <p className="text-sm text-gray-700 whitespace-pre-wrap">{offer.internal_note}</p>
              </div>
            )}
          </div>
        )}
        ...
        {(offer.lead_id || offer.client_id) && (
          <div className="card p-4">
            ...
            <div className="flex flex-wrap gap-2">
              {offer.lead_id && (
                <button onClick={() => navigate(`/sales/leads/${offer.lead_id}`)}
                  ...
                  <ExternalLink size={12} />Otwórz leada
                </button>
              )}
              {offer.client_id && (
                <button onClick={() => navigate(`/clients/${offer.client_id}`)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-green-600 bg-green-50 hover:bg-green-100 transition-colors">
                  <ExternalLink size={12} />Otwórz klienta
                </button>
              )}
            </div>
          </div>
        ...
        <div className="card p-4 opacity-60">
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="text-xs font-medium text-muted-500 uppercase tracking-wide">Eksport PDF</p>
          </div>
